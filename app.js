...
function campaignPracticeEstimateHtml(){
  const e=campaignPracticeEstimate(),h=e.hours<10?e.hours.toFixed(1):Math.round(e.hours).toString(),hl=e.hoursLow<10?e.hoursLow.toFixed(1):Math.round(e.hoursLow).toString(),hh=e.hoursHigh<10?e.hoursHigh.toFixed(1):Math.round(e.hoursHigh).toString();
...
function renderCampaign2Readiness(){
  ...
  if(box){
    ...
    if(estimateEl){
      ...
      estimateEl.innerHTML=estimate.completed?`<b>CAMPAIGN CLEARED</b>`:estimate.eligible?`<b>FINAL CHALLENGE READY</b>`:`<strong>~${h} h</strong><span>estimated focused practice left</span><b>~ ${estimate.days} days at ${Math.round(estimate.paceMinutes)} min/day</b><small>range ${hl}–${hh} h · ${estimate.mainGate}${estimate.calendarFloor?` · minimum ${estimate.calendarFloor} calendar days`:""}</small>`;
    }
