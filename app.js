...
function renderStatsScreen(){
  ...
  [["statsCoverage",st.coverage,true],["statsMastery",st.mastery,true],["statsAccuracy",st.accuracy,true],["statsAutomatic",st.auto,true]].forEach(([id,v,pc])=>{$(id).textContent=pc?`${pct(v)}%`:String(v);paintText(id,v);});$("statsAllAccuracy").textContent=st.allTimeAttempts?`${(st.allTimeAccuracy*100).toFixed(1)}%`:"—";paintText("statsAllAccuracy",st.allTimeAccuracy);$("statsAvg").textContent=st.avgMs?fmtSec(st.avgMs):"—";$("statsTotal").textContent=(state.totalAttempts||0).toLocaleString();$("statsStudyTime").textContent=formatStudyTime(state.activeTrainingMs||0);
...
function renderStart(){
  ...
  $("startAllAccuracy").textContent=st.allTimeAttempts?`${(st.allTimeAccuracy*100).toFixed(1)}%`:"—";paintText("startAllAccuracy",st.allTimeAccuracy);
