...
function sessionErrorGroupsPayload(records){
  return levelErrorGroups(records).map(g=>{const lesson=(window.AE_LESSONS||{})[g.cat]||{},coach=(window.AE_ERROR_COACH||{})[g.cat]||{},mis={};for(const r of g.records){const m=misconceptionFingerprint(r),x=mis[m.id]||(mis[m.id]={id:m.id,label:m.label,count:0,evidence:m.evidence,historicalCount:m.count,recentCount:m.recentCount});x.count++;}return {skill:skillLabel(g.cat),categoryId:g.cat,misses:g.records.length,formula:learningTerminology(lesson.formula||g.records[0]?.rule||""),quickRule:learningTerminology(lesson.cue||coach.must||""),spanishContrastPrompt:"Explain this group contrastively when supported: Spanish mental pattern/calque → why it tempts me → English pattern. If the evidence looks like a speed/attention slip instead, say so rather than forcing an L1 explanation.",misconceptions:Object.values(mis),examples:g.records.map(r=>({question:r.question||r.originalQuestion||"",myAnswer:r.userAnswer||"No answer",correctAnswer:r.correctAnswer||"",responseTimeSec:+((r.ms||0)/1000).toFixed(2),errorType:r.type||"wrong"}))};});
}
