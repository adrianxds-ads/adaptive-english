...
function campaignPracticeEstimate(){
  ...
  const hours=state.completed?0:Math.min(120,driver.hours),paceMinutes=todayMin>=10?todayMin:recommendedMin,paceBasis=todayMin>=10?"TODAY":"RECOMMENDED",practiceDays=Math.max(0,Math.ceil(hours*60/Math.max(1,paceMinutes))),calendarFloor=Math.max(0,Math.ceil(14-g.spanDays)),days=Math.max(practiceDays,calendarFloor);
