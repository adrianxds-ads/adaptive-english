...
function coachPromptText(){
  const st=overallStats(),ai=aiValorationStats(),learning=learningScoreStats(),peer=typicalLearnerStats(),c2=campaign2Readiness(),eta=campaignEta(),focus=coachSkillStats().slice(0,5),mistakes=commonMistakeGroups(8),trend=coachTrendSummary(),moves=coachSkillMovement(),targetPerf=targetPerformanceStats(),readingLoad=readingLoadStats(),focusTime=focusSummary();
...
function campaignEta(){
  const c=campaign2Readiness(),st=overallStats(),g=c.graduation,hist=state.history||[],sessions=(state.sessionHistory||[]).filter(x=>x.mode==="training"),now=Date.now(),day=86400000,learning=learningScoreStats(),focus=focusSummary();
  ...
  const observedPace=clamp(.006+longitudinalGain*.45+Math.max(0,recentTrend)*.25,.006,.035);
  const modelDaysAtRecommended=distance/Math.max(.005,observedPace),practiceMinutes=Math.max(0,modelDaysAtRecommended*recommendedMin);
  const paceMinutes=todayMin>=10?todayMin:recommendedMin,paceBasis=todayMin>=10?"TODAY":"RECOMMENDED";
  const practiceDays=Math.max(0,Math.ceil(practiceMinutes/Math.max(1,paceMinutes))),calendarFloor=Math.max(0,Math.ceil(14-g.spanDays)),days=Math.max(practiceDays,calendarFloor);
  const confidence=clamp((spanDays-5)/18)*clamp(sessions.length/100),spreadFactor=.48-.23*confidence,hours=practiceMinutes/60,hoursLow=Math.max(0,hours*(1-spreadFactor)),hoursHigh=hours*(1+spreadFactor);
  const daySpread=Math.max(2,Math.round(Math.max(1,practiceDays)*spreadFactor)),low=Math.max(calendarFloor,Math.max(0,practiceDays-daySpread)),high=Math.max(calendarFloor,practiceDays+daySpread);
  ...
  const enough=spanDays>=5&&sessions.length>=20,confidenceLabel=confidence>=.65?"HIGH":confidence>=.30?"MEDIUM":"LOW";
  if(state.completed)mainGate="CLEARED";else if(st.eligible)mainGate="FINAL CHALLENGE";
  return {days:state.completed?0:days,low:state.completed?0:low,high:state.completed?0:high,practiceDays:state.completed?0:practiceDays,calendarFloor:state.completed?0:calendarFloor,hours:state.completed?0:hours,hoursLow:state.completed?0:hoursLow,hoursHigh:state.completed?0:hoursHigh,practiceMinutes:state.completed?0:practiceMinutes,paceMinutes,paceBasis,recommendedMin,mainGate,enough,spanDays,todayMin,due,dueRatio,score:c.score,confidence,confidenceLabel,eligible:st.eligible,completed:state.completed};
}
function campaignEtaHtml(){
  const e=campaignEta(),h=e.hours<10?e.hours.toFixed(1):Math.round(e.hours).toString(),hl=e.hoursLow<10?e.hoursLow.toFixed(1):Math.round(e.hoursLow).toString(),hh=e.hoursHigh<10?e.hoursHigh.toFixed(1):Math.round(e.hoursHigh).toString();
  const headline=e.completed?"CLEARED":e.eligible?"FINAL":`~${h} h`;
  const sub=e.completed?"Campaign 1 completed.":e.eligible?"Graduation gates passed · final challenge remains.":`≈ ${e.days} calendar/practice days · ${Math.round(e.paceMinutes)} min/day (${e.paceBasis.toLowerCase()} pace)`;
  return `<article class="campaign-eta"><div><small>CAMPAIGN 2 · ESTIMATED PRACTICE LEFT</small><h2>${headline}</h2><p>${sub}</p><div class="campaign-eta-range">MODELED RANGE · ${hl}–${hh} h${e.calendarFloor?` · calendar floor ${e.calendarFloor}d`:""} · confidence ${e.confidenceLabel}</div></div><aside><span>MAIN GATE</span><b>${escapeHtml(e.mainGate)}</b><em>${e.due.toLocaleString()} reviews due</em></aside><footer>Hours = estimated focused app practice still needed, not a promise. Days use today's pace once you have ≥10 active minutes today; otherwise they use the recommended daily target. Mandatory calendar/retention gates can keep the campaign locked even after the estimated practice hours are completed.</footer></article>`;
}
...
function renderCampaign2Readiness(){
  const r=campaign2Readiness(),e=campaignEta(),score=pct(r.score),fill=$("campaign2Fill"),box=$("campaign2Box");
  if(box){$("campaign2Score").textContent=`${score}%`;paintText("campaign2Score",r.score);fill.style.width=`${score}%`;paintFill("campaign2Fill",r.score);$("campaign2Stage").textContent=r.stage;$("campaign2Status").textContent=r.ready?"Graduation gate achieved. The final challenge still decides Campaign 1 completion.":`${r.blockers.slice(0,2).join(" · ") || "Keep training to build stronger evidence."}`;const eta=$("campaign2Eta");if(eta){const h=e.hours<10?e.hours.toFixed(1):Math.round(e.hours),hl=e.hoursLow<10?e.hoursLow.toFixed(1):Math.round(e.hoursLow),hh=e.hoursHigh<10?e.hoursHigh.toFixed(1):Math.round(e.hoursHigh);eta.innerHTML=e.completed?`<b>CAMPAIGN CLEARED</b>`:e.eligible?`<b>FINAL CHALLENGE READY</b>`:`<strong>~${h} h</strong><span>estimated focused practice left</span><b>≈ ${e.days} days at ${Math.round(e.paceMinutes)} min/day</b><small>range ${hl}–${hh} h · ${e.mainGate}${e.calendarFloor?` · min ${e.calendarFloor} calendar days`:""}</small>`;}$("campaign2Copy").classList.toggle("hidden",!r.ready);box.classList.toggle("ready",r.ready);}
